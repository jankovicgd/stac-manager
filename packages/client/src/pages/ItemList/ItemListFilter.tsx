...
function ItemListFilter({
  ...
  const { isOpen, onClose, getDisclosureProps, getButtonProps } =
    useDisclosure();
  const buttonProps = getButtonProps();
  const disclosureProps = getDisclosureProps();
  const [buttonLabel, setButtonLabel] = useState('Filter items');

  const handleSelectCollection: React.ChangeEventHandler<HTMLSelectElement> = (
    event
  ) => {
  ...
  const handleDateRangeFrom = (value: string) =>
    setDateRangeFrom(value ? `${value}T00:00:00Z` : '');
  const dateRangeToValue = dateRangeTo?.split('T')[0];
  const handleDateRangeTo = (value: string) =>
    setDateRangeTo(value ? `${value}T00:00:00Z` : '');
  const rangeError =
    !!dateRangeTo && !!dateRangeFrom && dateRangeFrom >= dateRangeTo;
      ...
      >
        ...
        <Box display='grid' gridTemplateRows='max-content 1fr' gap='2'>
          <Box display='flex'>
            {isDrawing ? (
              <Text p='0' m='0'>
                Click on the map and drag to draw selected area.
              </Text>
            ) : (
              <>
                <Button variant='link' onClick={() => setIsDrawing(true)} p='0'>
                  {bbox ? 'Replace' : 'Add'} bounding box filter
                </Button>
                {bbox && (
                  <IconButton
                    variant='link'
